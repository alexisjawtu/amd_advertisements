...
include ("textos.php");
 if(empty($_pagi_cuantos)){
	// Si no se especifica, la cantidad de registros por página será por defecto 15
	$_pagi_cuantos = 15;
 }
 ...
 if(!isset($_pagi_nav_anterior)){
	$_pagi_nav_anterior = "&laquo; $paginado1";
 }
 if(!isset($_pagi_nav_siguiente)){
	$_pagi_nav_siguiente = "$paginado2 &raquo;";
 } 
 if(!isset($_pagi_nav_primera)){
	$_pagi_nav_primera = "&laquo;&laquo; $paginado3";
 } 
 if(!isset($_pagi_nav_ultima)){
	$_pagi_nav_ultima = "$paginado4 &raquo;&raquo;";
 } 
/*página actual*/
if (empty($_GET['pg'])) {
	// Si es la primera vez que se ejecuta el script en la sesión, $_pagi_actual es la pagina actual, por defecto, la primera.
	$_pagi_actual = 1;
}
else {
...
if($_pagi_conteo_alternativo == false && $_pagi_mostrar_errores == true) {
...
<script type = "text/javascript"> 
 ...
 if ($_pagi_totalReg < 1) header("Location:default.php");
 $_pagi_totalPags = ceil($_pagi_totalReg / $_pagi_cuantos);
if ($archivo_que_llama_paginado == 1) {
	$_pagi_enlace = "book_listing.php"; // en las bifurcaciones pongo music_listing o cualquiera que sea el producto.
}
$_pagi_query_string = "?";
...
if ($_pagi_totalReg > $_pagi_cuantos) { //ESTA SE MUESTRA
...
if($_pagi_hasta > $_pagi_totalReg){
 ...
 if ($_pagi_totalReg > $_pagi_cuantos) { //ESTA SE MUESTRA
 $_pagi_info = "<font face = \"arial\" size = \"2\">$paginado5 $_pagi_desde $paginado6 $_pagi_hasta $paginado7 $_pagi_totalReg</font>";
}
?>