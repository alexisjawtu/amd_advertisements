<?php
	...
	$i = 0;
	while ($array = fgetcsv ($handle1, 150, ';')) {
		if ($array[0]) {
			$matriz [] = $array;
			$array_productos [] = $array[1];
			$nombre_indice_temporal = $array[1];
			$array_medidas_de_cada_producto [$nombre_indice_temporal][] = $array[2];
			$nombre_indice_temporal2 = $array[2];
			$array_productos_de_cada_medida [$nombre_indice_temporal2][] = $array[1];
			$i ++;
		}
	}
	sort (&$matriz);
	...
	while ($matriz[$iterador][0]) {
		$medida = str_replace ('r', '-r', strtolower($matriz[$iterador][2]));																
		$string_nombre_archivo = str_replace (' ', '-', strtolower ('neumaticos-' . $medida . '-' . $matriz[$iterador][1]));
		...
		if (!is_file ('autos/' . $string_nombre_archivo . '.htm') ) {
			$archivo = fopen ('autos/' . $string_nombre_archivo . '.htm', "w"); //a los repetidos los pisa y el que queda es el segundo caso nada más
...
<meta name=\"keywords\" content=\"NEUMATICO " .  $matriz[$iterador][2] . " " . strtoupper ($matriz[$iterador][1]) . " - NEUMATICOS GOODYEAR " . 
	$medida_separada_para_imprimir . "\">
<meta name=\"description\" content=\"NEUMATICO " . $matriz[$iterador][2] . " " . strtoupper ($matriz[$iterador][1]) . " - NEUMATICOS GOODYEAR " .
	$medida_separada_para_imprimir . "\">
<title>NEUMATICO " . $matriz[$iterador][2] . " " . strtoupper ($matriz[$iterador][1]) . " - NEUMATICOS GOODYEAR " . $medida_separada_para_imprimir . "</title>
<style type=\"text/css\">
body
{
font-family: Arial; 
...
<div align=\"center\">
  ...
  <table cellspacing=\"0\" id=\"AutoNumber1\" bordercolor=\"#111111\" border=\"0\" height=\"89\" cellpadding=\"6\" width=\"600\" style=\"border-collapse: collapse\">
    <tr>
      ...
      GOODYEAR<br>" . $matriz[$iterador][2] . " " . strtoupper ($matriz[$iterador][1]) . "</font></p></td>
    </tr>
  </table>
  </center>
</div>
<div align=\"center\">
  ...
  <table cellspacing=\"0\" id=\"AutoNumber3\" bordercolor=\"#111111\" border=\"0\" cellpadding=\"6\" width=\"600\" style=\"border-collapse: collapse\">
    ...
    <tr>
      <td width=\"241\">
      <p align=\"center\">
      <a href=\"" . $matriz[$iterador][13] . "\">
      <img src=\"" . $matriz[$iterador][12] . "\" border=\"0\" alt=\"GOODYEAR NEUMATICOS" . strtoupper ($matriz[$iterador][1]) . "\"></a></p></td>
      <td width=\"359\">
      <p align=\"left\">
      <font face=\"Arial\" size=\"2\">
      <img src=\"cuadrado-negro.gif\" border=\"0\" height=\"12\" width=\"11\"> " . $matriz[$iterador][2] . " " . 
      "<a href=\"" . $matriz[$iterador][13] . "\">" . strtoupper ($matriz[$iterador][1]) . "</a></font></p></td>
    </tr>
  </table>
  <p align=\"left\">&nbsp;</p>
  </center>
...
<div align=\"center\">
  ...
  <div align=\"center\">
    <table border=\"0\" cellpadding=\"6\" cellspacing=\"0\" width=\"600\" id=\"table2\">
      <tr>
		<td>
         <font face=\"Arial\" size=\"2\" color=\"#00923F\">Otros modelos en " . $matriz[$iterador][2] . "</font></td>
	  </tr>
	  <tr><td style=\"border-bottom-style: solid; border-bottom-width: 1px\">";
			foreach ($matriz_productos_de_cada_medida[$matriz[$iterador][2]] as $value) {
				// falta a href!
	  ...
	  <tr>
        <td>
         <font face=\"Arial\" color=\"#00923F\">Otras medidas de " . $matriz[$iterador][1] . "</font></td>
	  </tr><tr><td>";	
			foreach ($matriz_medidas_de_cada_producto[$matriz[$iterador][1]] as $value) {
				$nombre_temp = str_replace (' ', '-', strtolower ($matriz[$iterador][1]));
				$nombre_temp = str_replace ('/', '-', $nombre_temp);
				$nombre_temp = str_replace ('ñ', 'n', $nombre_temp);
				$nombre_temp = str_replace ('á', 'a', $nombre_temp);
				$nombre_temp = str_replace ('é', 'e', $nombre_temp);
  ...
  <table cellspacing=\"0\" border=\"0\" height=\"55\" cellpadding=\"0\" width=\"600\">
    ...
    <tr>
      ...
      <form action=\"http://www.viarrural.com.ar/local-cgi/FormMail.pl\" method=\"post\">
        ...
        <input name=\"subject\" type=\"hidden\" value=\"GOODYEAR - " . strtoupper ($matriz[$iterador][1]) . " - " . $matriz[$iterador][2] . "\">
        <div align=\"center\">
          <div align=\"center\">
